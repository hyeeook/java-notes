...
> 💡 static 메서드도 정적 바인딩이다. 자식이 부모와 같은 시그니처의 static 메서드를 정의하면 오버라이딩이 아니라 hiding(감추기)이며, 호출되는 메서드는 실제 객체 타입이 아니라 참조 변수의 선언 타입으로 결정된다. 그래서 static 메서드에는 `@Override`를 붙일 수 없다.
